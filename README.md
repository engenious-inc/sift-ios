...
| `testsExecutionTimeout` | Per-test execution time allowance in seconds (default 300), injected into the xctestrun as XCTest's default/maximum test execution time allowance. XCTest rounds it up to whole minutes; a test that exceeds it is failed, the runner is restarted and the run continues, normally with a readable result bundle. The wall-clock budget after which Sift terminates a hung `xcodebuild` (TERM→KILL, status 143) is derived from it, never equal to it: a conservative `300 + tests-in-bucket × (rounded allowance + 60)` seconds (capped at ten years), sized so that XCTest's own per-test timeout normally fires first for every test in the bucket. Startup, runner restarts or bundle finalization that outgrow those overheads still end in a kill; the per-chunk budget breakdown is logged in verbose mode. |
| `setUpScriptPath` / `tearDownScriptPath` | Optional scripts run on the node before/after each bucket. Uploaded as 0700 files and executed directly — the shebang is honored. Env: `TEST_NAME`, `TEST_MANIFEST` (path to a newline-delimited test list), `UDID`, plus your `environmentVariables` (`TEST_NAMES` is deprecated, kept one release). A nonzero setup exit returns the bucket to the queue; a nonzero teardown is reported as a health event. Each script runs as a Sift-owned process bounded by `testsExecutionTimeout` itself (unrounded; terminated on expiry, status 143), and a setup script is terminated when the run is cancelled; a helper a script leaves running in the background must redirect its output (`helper >/dev/null 2>&1 &`). |
